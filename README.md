...
### 中文字幕跑到一半就沒了、或完全沒有說明動畫

**幾乎都是 Claude 用量到上限。** 那些呼叫在中途整批失敗，pipeline 會警告但繼續
跑完，所以表面上看起來像成功。等額度恢復後重跑即可：

```bash
python run.py --only 04      # 補中文字幕
python run.py --only 05 06   # 補說明動畫並重組時間軸
```

已經完成的部分會命中快取，只會重問缺的那些，不會重複消耗額度。

現在 stage 04 遇到整批失敗會自動用更小的批次重試兩輪；真的補不齊時會明確列出
缺口的時間範圍。跑完之後 `run.py` 也會比對中英字幕的數量，不一致就在結尾提醒。

### 「嗯嗯」「Mm-hmm」不需要字幕

預設就會拿掉 —— 整則字幕只有應答語時直接不顯示：
...
只影響字幕，**不會剪掉影片**（那些聲音常和對方說話重疊，剪掉會破壞聲音）。
`yeah` / `okay` / `right` 刻意保留，它們常常是有意義的回答。
