"""
Stage 06：把剪輯決策組成一份 FCPXML，匯入 Final Cut Pro 就是一條剪好的時間軸。

時間軸結構：
  spine        每個保留片段一個 asset-clip（停頓與結巴已經被切掉）
    lane 1     航線／名詞說明短片（含 alpha 通道，直接疊在畫面上）
    lane -1    字幕（選用；YouTube 的隱藏式字幕請用輸出的 .srt）
"""
...
class _Res:
    ...
    def _apply_timecode_override(self, path: str, info: dict) -> None:
        ...
        if parsed is None:
            warn("06", f"{name} 的時間碼覆寫值 {tc!r} 格式不對，忽略")
            return
        info["start"], info["drop_frame"] = parsed
        info["timecode"] = tc
        log("06", f"{name} 使用手動指定的時間碼 {tc} → {parsed[0]:.3f}s")
...
def run_stage(cfg, claude=None) -> dict:
    ...
    expl = read_json(cfg.build_file("05_explainers.json"), default={"items": []})
    ...
    if media_start:
        log("06", f"訪談影片時間碼 {main_info.get('timecode')}（{tc_format}）"
                  f"→ 素材起點 {media_start:.3f}s")
    elif main_info.get("timecode"):
        warn("06", f"讀到時間碼 {main_info['timecode']} 但換算成 0s，格式可能不認得")
    else:
        log("06", "訪談影片沒有嵌入時間碼，素材起點 = 0s")
    ...
    for it in expl.get("items", []):
        try:
            ...
            warn("06", f'說明短片 {it["path"]} 無法加入：{exc}')
            continue
        c = owner(float(it["at"]))
    ...
    if grid:
        warn("06", f"有 {len(grid)} 個項目沒有對齊序列的影格網格，"
    ...
    if problems:
        warn("06", f"有 {len(problems)} 個 clip 取用的範圍超出素材的媒體範圍，"
                   "Final Cut Pro 會說「沒有個別媒體，剪輯無效」：")
        for line in problems[:5]:
            warn("06", f"  {line}")

    out = cfg.build_file("06_timeline.fcpxml")
    _write(root, out)
    log("06", f"FCPXML 完成：{len(clips)} 段主畫面、{n_expl} 段說明短片、"
              f"{n_cap} 則字幕")
    log("06", f"成品長度 {fmt_hhmmss(total)} → {out}")
    return {"path": str(out), "duration": total, "clips": len(clips),
            "explainers": n_expl, "captions": n_cap}
